...
export function validatePrismUploadStructure(uploadData, options = {}) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            if (normalizedEntry.hardware !== rootHw && normalizedEntry.hardware !== 'Unknown') {
            ...
            if (entry.run_uid && format === 'brv02' && parsedStage.runUid !== entry.run_uid) {
                errors.push(`Stage ${stageIndex} (${entry.filename}) has mismatching run UID: expected '${entry.run_uid}', but found '${parsedStage.runUid}'`);
            }
